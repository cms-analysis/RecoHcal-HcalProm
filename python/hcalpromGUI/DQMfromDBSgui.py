...
#
# v1.2 Beta version allows access from non-hcal DQMs
#################################################
...
class DQMDBSgui:
    ...
    def callDQMscript(self,i):
        ...
        if os.path.isdir(os.path.join(self.basedir,"prompt_out.root")):
            success=True
            newname="prompt_out_%i.root"%i
            os.system("mv %s %s"%(os.path.join(self.basedir,"prompt_out.root"),
                                  os.path.join(self.basedir,newname)))
            # Move to a new directory once final_dir is determined
            if (self.finalDir.get()<>self.basedir):
                os.system("mv %s %s"%(os.path.join(self.basedir,newname),
                                      os.path.join(self.finalDir.get(),newname))) 
